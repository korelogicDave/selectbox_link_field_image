<?php
	...
	Class Extension_Selectbox_Link_Field_Image extends extension_selectbox_link_field
	{
		...
		public function install(){
			try{
				Symphony::Database()->query("CREATE TABLE IF NOT EXISTS `tbl_fields_selectbox_link_image` (
                  ...
                  `hide_when_prepopulated` ENUM( 'yes', 'no' ) NOT NULL default 'yes',
              PRIMARY KEY  (`id`),
		...
		public function uninstall(){
			Symphony::Database()->query("DROP TABLE `tbl_fields_selectbox_link_image`");
		...
		public function update($previousVersion){
			if( version_compare($previousVersion, '1.3', '<') ){
				...
				Symphony::Database()->query("ALTER TABLE `tbl_fields_selectbox_link_image` ADD `show_created` INT(1) NOT NULL");

				// By default, save all the current relations as being created by their parent entry (which is often the case):
				$field_ids = Symphony::Database()->fetchCol('field_id', 'SELECT `field_id` FROM `tbl_fields_selectbox_link_image`;');
			...
			if( version_compare($previousVersion, '1.5', '<') ){
				Symphony::Database()->query("
				ALTER TABLE  `tbl_fields_selectbox_link_image`
